...
export async function executeDiagnostics(args: ToolArguments, onProgress?: (newOutput: string) => void): Promise<string> {
  const typedArgs = args as z.infer<typeof diagnosticsSchema.inputSchema>;
...
function determineDiagnosticStatus(diagnostics: any): {
